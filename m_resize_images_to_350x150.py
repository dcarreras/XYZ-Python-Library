...
def resize_images_to_350x150(image_paths, listbox):
    resized_images = []
    for image_path in image_paths:
        try:
            with Image.open(image_path) as img:
                # Resize image to 350x150
                resized_img = img.resize((350, 150), Image.Resampling.LANCZOS)
                # Overwrite the original image or save to a new file if preferred
...
def select_images_and_resize():
    ...
    if file_paths:
        ...
        resized_images = resize_images_to_350x150(file_paths, listbox)
...
# Create a label
label = Label(root, text="Resize Images to 350x150", font=("Helvetica", 14))
label.pack(pady=10)
